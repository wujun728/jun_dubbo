$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'good/goodimage/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.goodImage.picImgId == null ? "good/goodimage/save" : "good/goodimage/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.goodImage),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "good/goodimage/delete",
                    contentType: "application/json",
		...
		getInfo: function(picImgId){
			$.get(baseURL + "good/goodimage/info/"+picImgId, function(r){
                vm.goodImage = r.goodImage;
            });
		},
