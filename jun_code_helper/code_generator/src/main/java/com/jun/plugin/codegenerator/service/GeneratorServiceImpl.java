package com.jun.plugin.codegenerator.service;
    ...
    public Map<String, String> getResultByParams(Map<String, Object> params) throws IOException, TemplateException {
        ...
        for (int i = 0; i <parentTemplates.size() ; i++) {
            ...
            for (int x = 0; x <parentTemplateObj.getJSONArray("templates").size() ; x++) {
                ...
                result.put(childTemplate.getString("name")+"_code", FreemarkerUtil.processString(parentTemplateObj.getString("group") + "/" +childTemplate.getString("name")+ ".ftl", params));
            }
        }
        return result;
    }
